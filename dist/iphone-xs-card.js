          ...
          <ha-card .hass="${this._hass}" .config="${this._config}" class="background" style="${this.style.background}" @click="${() => this.fireEvent('hass-more-info')}">
            <div class="content" >
                ...
                <div class="grid-container">
                  <div class="icon"><img class="device-type image-invert" src="/hacsfiles/iphone-xs-card/device_type.png" alt="Type" /></div>
                  <div class="text"><span>${this.getValueState('device_type')}</span></div>
                </div>

                ${this.state.showSim1 ? html`
                <div class="grid-container">
                  <div class="icon"><img class="sim image-invert" src="/hacsfiles/iphone-xs-card/sim_1.png" alt="Sim1" /></div>
                  <div class="text"><span>${this.getValueState('sim_1')}</span></div>
                </div>` : null}
                ${this.state.showSim2 ? html`
                <div class="grid-container">
                  <div class="icon"><img class="sim image-invert" src="/hacsfiles/iphone-xs-card/sim_2.png" alt="Sim2" /></div>
                  <div class="text"><span>${this.getValueState('sim_2')}</span></div>
                </div>` : null}
               
                ${this.state.showWifi ? html`
                <div class="grid-container">
                  <div class="icon"><img class="wifi image-invert" src="/hacsfiles/iphone-xs-card/wifi.png" alt="WiFi" /></div>
                  <div class="text"><span>${this.getValueState('wifi_on')}</span></div>
                </div>` : null}

                <div class="battery-box" >
                  <img class="battery image-invert" src="/hacsfiles/iphone-xs-card/battery.png" alt="Bat" />
                  <div class="battery-fill-box">
                    <div class="battery-fill" style="width: ${this.getValueState('battery')}%; background-color: ${this.getBatteryColor()};"></div>
                ...
                <div class="plug-box" style="${this.getValueState('battery_charging') !== 'true' ? 'display: none;' : ''}">
                  <img class="plug image-invert" src="/hacsfiles/iphone-xs-card/plug.png" alt="Plug" />
                </div>
                ` : null}
            </div>
          </ha-card>`;
